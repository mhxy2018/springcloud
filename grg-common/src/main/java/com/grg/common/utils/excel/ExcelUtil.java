package com.grg.common.utils.excel;

import com.grg.common.annotation.Excel;
import com.grg.common.annotation.Excels;
import com.grg.common.core.text.Convert;
import com.grg.common.enumeration.ResponseCode;
import com.grg.common.exception.ConsumerException;
import com.grg.common.utils.DateUtils;
import com.grg.common.utils.StringUtils;
import com.grg.common.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T> {
    ...
    private void init(List<T> list, String sheetName, Excel.Type type) {
        ...
        this.type = type;
        log.debug("excel表格导出 - ExcelUtil初始化");
        createExcelField();
        createWorkbook();
    }
    ...
    public List<T> importExcel(InputStream is) throws Exception {
        try {
            ...
            log.error("导入Excel异常{}", ExceptionUtils.getStackTrace(e));
            throw new ConsumerException(ResponseCode.IMPORT_ERROR);
        }
    }
     ...
     * @return ResponseCode
     */
    public ResponseCode exportExcel(List<T> list, HttpServletResponse resp, String sheetName) {
        try {
            ...
            return ResponseCode.EXPORT_ERROR;
        }
     ...
     * @return 结果
     */
    public ResponseCode importTemplateExcel(HttpServletResponse resp, String sheetName) {
        try {
            ...
            log.error("导入Excel异常{}", ExceptionUtils.getStackTrace(e));
            return ResponseCode.IMPORT_ERROR;
        }
    }
     ...
     * @param resp 导出路径
     */
    private void exportExcel(HttpServletResponse resp) {
        ...
        try {
            ...
            log.debug("excel导出 - HttpServletResponse响应设置");
            ...
            log.debug("excel导出 - 导出准备");
            out = resp.getOutputStream();
            wb.write(out);
            log.debug("excel导出 - 导出成功");
        } catch (Exception e) {
            log.error("导出Excel异常{}", ExceptionUtils.getStackTrace(e));
            throw new ConsumerException(ResponseCode.EXPORT_ERROR);
        } finally {
            if (wb != null) {
                try {
                    wb.close();
                } catch (IOException e1) {
                    log.error("导出Excel异常,关闭工作表失败{}", ExceptionUtils.getStackTrace(e1));
                }
            }
            if (out != null) {
                try {
                    ...
                    log.error("导出Excel异常,关闭输出流失败{}", ExceptionUtils.getStackTrace(e1));
                }
            }
        }
    }
    ...
    private Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            log.error("导出Excel异常,设置单元格失败{}", ExceptionUtils.getStackTrace(e));
            throw new ConsumerException(ResponseCode.EXPORT_ERROR);
        }
        return cell;
    }
    ...
    private static String convertByExp(String propertyValue, String converterExp) throws Exception {
        try {
            ...
            log.error("导出Excel异常,解析导出参数失败{}", ExceptionUtils.getStackTrace(e));
            throw new ConsumerException(ResponseCode.EXPORT_ERROR);
        }
    ...
    private static String reverseByExp(String propertyValue, String converterExp) {
        try {
            ...
            log.error("导出Excel异常,反向解析参数失败{}", ExceptionUtils.getStackTrace(e));
            throw new ConsumerException(ResponseCode.EXPORT_ERROR);
        }
        return propertyValue;
    }
    ...
    private String encodingFilename(String filename) {
        filename += "_" + UUID.randomUUID().toString().substring(0,6) + ".xlsx";
        return filename;
    }
